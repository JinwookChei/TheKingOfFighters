...
void Iori::Initialize(const Vector& position, bool useCameraPosition, bool flip) {
  ...
  pRender_->CreateAnimation(IOAS_GaishikiMutan_1, 3, 99, 107, 50, false, 99);      // 외식 몽탄_1
  pRender_->CreateAnimation(IOAS_GaishikiMutan_2, 3, 160, 164, 50, false, 160);    // 외식 몽탄_2
  pRender_->CreateAnimation(IOAS_Shinigami, 3, 136, 144, 50, false, 136);          // 외식 광부 음 "사신"
  pRender_->CreateAnimation(IOAS_HyakushikiOniyaki, 3, 276, 291, 50, false, 276);  // 백식 귀신 태우기
  pRender_->CreateAnimation(IOAS_127ShikiAoiHana_1, 3, 255, 261, 50, false, 255);  // 백식 귀신 태우기
  pRender_->CreateAnimation(IOAS_127ShikiAoiHana_2, 3, 262, 268, 50, false, 262);  // 백식 귀신 태우기
  pRender_->CreateAnimation(IOAS_127ShikiAoiHana_3, 3, 269, 275, 50, false, 269);  // 백식 귀신 태우기
