...
void Chang::Initialize(bool isPlayer1, const Vector& position, bool useCameraPosition, KOFPlayer* opponentPlayer) {
  ...
  AnimTransRule animTransRule_TekkyuuGeki;
  animTransRule_TekkyuuGeki.transCondition_ = ANIM_TRANS_COND::MovementOnGround;
  animTransRule_TekkyuuGeki.toAnimState_ = PLAYER_ANIMTYPE_JumpLand;
  AnimTransState animState_TekkyuuGeki;
  animState_TekkyuuGeki.fromAnimState_ = CHANG_ANIMTYPE_TekkyuuGeki;
  animState_TekkyuuGeki.animTransRules_.push_back(animTransRule_TekkyuuGeki);
  pAnimaStateTransMachine_->RegistAnimTransition(animState_TekkyuuGeki);
