...
class TimeManager {
 ...
 public:
  ...
  ENGINE_API static TimeManager* Instance();

  ENGINE_API void OnFrameFreeze(unsigned long long freezeDuration = 200);

  bool FrameFreeze(unsigned long long deltaTick);

  ENGINE_API void SetTimeScale(float scale, float lerpSpeed = 5.0f);
